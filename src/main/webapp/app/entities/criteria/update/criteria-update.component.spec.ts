...
import { ActionParameterService } from 'app/entities/action-parameter/service/action-parameter.service';
...
describe('Criteria Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call ActionParameter query and add missing value', () => {
      const criteria: ICriteria = { id: 456 };
      const actionParameters: IActionParameter[] = [{ id: 8719 }];
      criteria.actionParameters = actionParameters;

      const actionParameterCollection: IActionParameter[] = [{ id: 14753 }];
      jest.spyOn(actionParameterService, 'query').mockReturnValue(of(new HttpResponse({ body: actionParameterCollection })));
      const additionalActionParameters = [...actionParameters];
      const expectedCollection: IActionParameter[] = [...additionalActionParameters, ...actionParameterCollection];
      jest.spyOn(actionParameterService, 'addActionParameterToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ criteria });
      comp.ngOnInit();

      expect(actionParameterService.query).toHaveBeenCalled();
      expect(actionParameterService.addActionParameterToCollectionIfMissing).toHaveBeenCalledWith(
    ...
    it('Should update editForm', () => {
      const criteria: ICriteria = { id: 456 };
      const actionParameters: IActionParameter = { id: 70505 };
      criteria.actionParameters = [actionParameters];
      ...
      expect(comp.actionParametersSharedCollection).toContain(actionParameters);
  ...
  describe('Compare relationships', () => {
    describe('compareActionParameter', () => {
      it('Should forward to actionParameterService', () => {
        ...
        jest.spyOn(actionParameterService, 'compareActionParameter');
        comp.compareActionParameter(entity, entity2);
        expect(actionParameterService.compareActionParameter).toHaveBeenCalledWith(entity, entity2);
      });
    });
