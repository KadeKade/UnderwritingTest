...
export class CriteriaUpdateComponent implements OnInit {
  ...
  actionParametersSharedCollection: IActionParameter[] = [];
  ...
  compareActionParameter = (o1: IActionParameter | null, o2: IActionParameter | null): boolean =>
    this.actionParameterService.compareActionParameter(o1, o2);
  ...
  protected updateForm(criteria: ICriteria): void {
    ...
    this.actionParametersSharedCollection = this.actionParameterService.addActionParameterToCollectionIfMissing<IActionParameter>(
  ...
  protected loadRelationshipsOptions(): void {
    this.actionParameterService
      .query()
      .pipe(map((res: HttpResponse<IActionParameter[]>) => res.body ?? []))
      .pipe(
        map((actionParameters: IActionParameter[]) =>
          this.actionParameterService.addActionParameterToCollectionIfMissing<IActionParameter>(
            actionParameters,
            ...(this.criteria?.actionParameters ?? [])
          )
        )
      )
      .subscribe((actionParameters: IActionParameter[]) => (this.actionParametersSharedCollection = actionParameters));
